...
export default function Food({ food, cellSize, isBeingEaten = false }: FoodProps) {
  const isSpecial = food.type !== 'normal'

  return (
    <div
      data-testid={`food-${food.id}`}
      className={`absolute rounded-full ${isBeingEaten ? 'animate-eat-food' : 'transition-all duration-200'
        } ${isSpecial && !isBeingEaten ? 'animate-pulse' : ''
        }`}
      style={{
